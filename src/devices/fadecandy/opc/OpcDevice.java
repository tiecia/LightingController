...
public class OpcDevice {
	...
	public PixelStrip addPixelStrip(int pinNumber, int pixelCount, int firstOutputPixel, String description) {
		assert pixelCount > 0 && pixelCount <= 64;
		assert pinNumber>=0 && pinNumber < 8;
		assert firstOutputPixel>=0 && firstOutputPixel <= 64-pixelCount;
		//Change: dynamic offset to fixed offset
		PixelStrip strip = new PixelStrip(this, pinNumber, pixelCount, firstOutputPixel, description);
